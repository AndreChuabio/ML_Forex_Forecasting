...
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score, mean_absolute_percentage_error, ConfusionMatrixDisplay, confusion_matrix
...
df = pd.read_csv('../../output/USDJPY_data.csv')
...
for days in range(1, 6):
    ...
    for name, estimator in base_models.items():  # build two step pipeline: scaler ->  model
        ...
        plt.title(f"{name} {days}-day ahead: Actual vs Predicted USDJPY")
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"results/{name}_{days}d_actual_vs_pred_USDJPY.png")
        plt.close()
        ...
        plt.title(f"{name} {days}-day ahead Sign Confusion_USDJPY")
        plt.savefig(f"results/{name}_{days}d_confusion_USDJPY.png")
        plt.close()
        ...
        plt.title(
            f"{days}-Day Ahead: Actual vs Predicted Returns for USDJPY ({name})")
        plt.xlabel("Date")
        plt.ylabel("Return")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"results/actual_vs_predicted_USDJPY_{name}_{days}d.png")
        plt.close()
       ...
       # Generic feature-selection: keep only top quartile of features by score
        ...
        print(
            f"\nUSDJPY Full feature importances for {name} ({days}-day ahead):")
        print(imp_df.to_markdown(index=False))

        imp_df.to_csv(
            f"results/{name}_{days}d_feature_importancesUSDJPY.csv",
            index=False
        )
        ...
        if days == 5 and name == 'XGB':
            ...
            print("\n Top 10 XGBoost Feature Importances USDJPY (5-day ahead):")
            for i in top_10:  # top 10 features
                print(f'{features[i]:20s}:{imps[i]:.4f}')


metrics_df = pd.DataFrame(all_metrics)  # Metrics table Viz
metrics_df.to_csv('results/metrics_by_horizonUSDJPY.csv', index=False)
...
for metric in ["R2", "RMSE", "MAE", "HitRate"]:
    ...
    plt.xlabel("Forecast horizon(days)for USDJPY")
    plt.ylabel(metric)
    plt.title(f"{metric} by horizon")
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{metric.lower()}_by_horizon_USDJPY")
    plt.close()
